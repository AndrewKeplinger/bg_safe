...
( function () {
	...
	Object.assign( THREE.FBXLoader.prototype, {
		...
		if (filename.indexOf(".zip")>-1){
					var loader = new JSZip();
				} else {
		*/
		loadzip: function ( url, onLoad, onProgress, onError ) {
			var self = this;
			var resourceDirectory = url.split( /[\\\/]/ );
			var filename = resourceDirectory[resourceDirectory.length-1];
			var isDemented = filename.indexOf(".dem")>-1;
			filename = filename.substring(0,filename.length-4);// trim off extra extension
			resourceDirectory.pop();
			resourceDirectory = resourceDirectory.join( '/' ) + '/';
			var loader = new THREE.FileLoader( this.manager );
			
			//loader.load( url, function ( ziptext ) {
			try {
				JSZipUtils.getBinaryContent(url, function (err, ziptext) {
					if(err) {
						//throw err; // or handle err
					}
					var zipObj = new JSZip();
					zipObj.loadAsync(ziptext).then ( function (zip) {
						zip.file(filename).async("string").then( function (text) {
							//console.log(text);
							if (isDemented) {
								//console.log("start:"+filename+"  "+globalLastLoad.substr(0,1000));//.length);
								//public string decode( string baseStr, string diff ) {
								var baseStr = globalLastLoad;
								var diff = text;
								var destIdx = 0;
								var baseIdx = 0;
								var outStr = "";
								var outBits = [];
								//self.lastLoad contains the 
								while (destIdx < diff.length) {
									// Decode in sections, each section 
									//console.log(diff.substr(0,500));
									var matchLength = fromAscIINum(diff.substr(destIdx,3));
									destIdx+=3;
									outBits.push(baseStr.substr (baseIdx,matchLength));// Math.min(matchLength,baseStr.Length-baseIdx)));
									if (destIdx + 6 > diff.length) {
										break;
									}
									var srcSkip = fromAscIINum(diff.substr(destIdx,3));
									destIdx += 3;
									var destLength = fromAscIINum(diff.substr(destIdx,3));
									//console.log(matchLength+"  "+srcSkip+"  "+destLength);
									destIdx += 3;
									outBits.push(diff.substr (destIdx, destLength));//Math.min(destLength,diff.length-destIdx)));
									destIdx += destLength;
									baseIdx += matchLength + srcSkip;
								}
								if (baseIdx<baseStr.length) {
									outBits.push( baseStr.substr (baseIdx) );
								}
								text = outBits.join("");
								//console.log("final:"+filename+"  "+text.substr(0,1000));
								
							}
							//console.log(text.substr(0,2000));
							var scene = self.parse( text, resourceDirectory );
							self.lastLoad = text;
							self.lastResPath = resourceDirectory;
							onLoad( scene );
